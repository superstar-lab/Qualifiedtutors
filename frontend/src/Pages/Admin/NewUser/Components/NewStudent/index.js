...
/**
 * Form for creating new student type users
 */
function NewStudent({...props}) {
    ...
    const save = async () => {
        ...
        if (!hasErrors) {
            try {
                const response = await API.post("register/client", params)
                
                if (response && response.data && response.data.success) {
                    Toast.success("Successfully created new student")
    ...
    return <Container>
        <Section className="section" data-label="Account" collapsed={sections.account} onClick={e => toggleSection(e, 'account')}>
            ...
            <Row>
                <RadioButton name="verificationStatus" value="pending" small current={verificationStatus} setter={value => setVerificationStatus(value)}>Pending</RadioButton>
                <RadioButton name="verificationStatus" value="approved" small current={verificationStatus} setter={value => setVerificationStatus(value)}>Approved</RadioButton>
            </Row>
        </Section>
        ...
        <Row style={{justifyContent: 'space-between', marginTop: '32px'}}>
            <div></div>
            <Button primary onClick={save}>{uploading ? <RingLoader small /> : 'Create student'}</Button>
        </Row>
    </Container>
}
